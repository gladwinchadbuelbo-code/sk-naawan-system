...
import { FileText, Wallet, CheckCircle, Clock, XCircle, MessageSquare, Eye, ArrowLeft, Filter } from 'lucide-react';
...
export function MyBudgetProposalsPage() {
  ...
  useEffect(() => {
    ...
    const allProposals = storage.getBudgetProposals();
    const myProposals = allProposals.filter(
      (p: BudgetProposal) => p.submittedBy === user?.fullName
    );
    ...
    return (
      <div className="p-8">
        <Card className="p-8 text-center">
          ...
          <p className="text-gray-600">Only SK Treasurer can view budget proposals.</p>
        </Card>
      </div>
    );
  }
  ...
  const handleViewDetails = (proposal: BudgetProposal) => {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <h1 className="text-3xl mb-2">My Budget Proposals</h1>
        <p className="text-gray-600">Track the status of your submitted budget proposals</p>
      </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 mb-4 line-clamp-2">{proposal.description || 'No description provided'}</p>

                    {/* Proposal Details */}
                    <div className="mb-4">
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                        <Wallet className="w-4 h-4" />
                        <span className="text-green-600 text-lg">₱{(proposal.totalAmount || 0).toLocaleString()}</span>
                      </div>
                      <div className="text-sm text-gray-600">
                        {proposal.items?.length || 0} budget item(s)
                      </div>
                    </div>
          ...
          <Card className="p-12 text-center">
            ...
            <p className="text-gray-600 mb-4">
              {searchTerm || statusFilter !== 'all'
                ...
                : 'You haven\'t submitted any budget proposals yet'}
            </p>
            {!searchTerm && statusFilter === 'all' && (
              <Button onClick={() => navigate('/staff/budget/proposal/create')}>
        ...
        <Dialog open={showDetailsModal} onOpenChange={setShowDetailsModal}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Complete details of your budget proposal
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-900">{selectedProposal.description || 'No description provided'}</p>
              </div>

              {/* Total Amount */}
              <div>
                <h3 className="text-sm text-gray-600 mb-1">Total Amount</h3>
                <p className="flex items-center gap-2 text-2xl text-green-600">
                  <Wallet className="w-6 h-6" />
                  ₱{(selectedProposal.totalAmount || 0).toLocaleString()}
                </p>
              </div>
              ...
              {selectedProposal.items && selectedProposal.items.length > 0 && (
                <div>
                  <h3 className="text-sm mb-2">Budget Items</h3>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y">
                        {selectedProposal.items.map((item, index) => (
                          <tr key={index}>
                            <td className="px-4 py-2 text-sm">{item.category || 'N/A'}</td>
                        ...
                        <tr className="bg-gray-50">
                          ...
                          <td className="px-4 py-2 text-sm text-right">
                            <strong>
                              ₱{selectedProposal.items.reduce((sum, item) => sum + (Number(item.amount) || 0), 0).toLocaleString()}
                            </strong>
                          </td>
                        </tr>
                      </tbody>
            ...
            <DialogFooter>
              ...
              {selectedProposal.status === 'returned' && (
                <Button onClick={() => {
                  ...
                  navigate('/staff/budget/proposal/create');
                }}>
                  Create Revised Proposal
                </Button>
              )}
